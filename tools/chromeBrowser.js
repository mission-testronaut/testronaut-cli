...
export class ChromeBrowser {
  constructor(options = {}) {
    ...
    this.domListLimit = options.domListLimit;
    this.debugEnabled = !!options.debug;
    this._domLimitLogged = false;

    const defaultResourceGuard = {
      enabled: true,
      hrefIncludes: ['/document/', '/file/', '/download', '/attachment/'],
      dataTypes: ['document', 'file', 'item', 'row'],
    };
    const guardCfg = options.resourceGuard || defaultResourceGuard;
    this.resourceGuard = {
      enabled: guardCfg.enabled !== false,
      hrefIncludes: Array.isArray(guardCfg.hrefIncludes)
        ? guardCfg.hrefIncludes.map(s => s.toLowerCase())
        : defaultResourceGuard.hrefIncludes,
      dataTypes: Array.isArray(guardCfg.dataTypes)
        ? guardCfg.dataTypes.map(s => s.toLowerCase())
        : defaultResourceGuard.dataTypes,
    };
  }
  ...
  async get_dom(
    ...
    const matchesHrefPattern = (href = '') => {
      const lower = (href || '').toLowerCase();
      return this.resourceGuard.hrefIncludes.some(p => lower.includes(p));
    };

    const extractInitialDocumentList = (rawHtml) => {
      const key = '"initialDocumentList":';
      const idx = rawHtml.indexOf(key);
      if (idx < 0) return [];
      const start = rawHtml.indexOf('[', idx);
      if (start < 0) return [];
      let depth = 0;
      let end = -1;
      for (let i = start; i < rawHtml.length; i++) {
        const ch = rawHtml[i];
        if (ch === '[') depth++;
        else if (ch === ']') depth--;
        if (depth === 0) {
      ...
      if (end === -1) return [];
      const jsonSlice = rawHtml.slice(start, end + 1);
      try {
    ...
    const normalizeListLimit = () => {
      if (this.domListLimit === Infinity) return Infinity;
      const asNum = Number(this.domListLimit);
      if (Number.isFinite(asNum)) {
        return Math.min(100, Math.max(0, asNum));
      }
      return 3; // current default behavior (keep first 3 matching items)
    };
    const listLimit = normalizeListLimit();
    if (this.debugEnabled && !this._domLimitLogged) {
      const pretty = listLimit === Infinity ? 'all' : listLimit;
      this._debugLog(`DOM list limit resolved`, { raw: this.domListLimit, applied: pretty });
      this._domLimitLogged = true;
    }
    if (this.debugEnabled) {
      const pretty = listLimit === Infinity ? 'all' : listLimit;
      this._debugLog(`get_dom invoked`, { limit, exclude, focus, listLimit: pretty });
    }
    ...
    if (exclude) {
      ...
      $ = await reduceRepeatedDivs($, listLimit);
    ...
    if (this.debugEnabled) {
      ...
      const ulSizes = [];
      $('ul').each((_, el) => {
        ulSizes.push($(el).children('li').length);
      });
      const maxUlSize = ulSizes.length ? Math.max(...ulSizes) : 0;
      const anchorDocs = $('a[href]').filter((_, el) => {
        if (!this.resourceGuard.enabled) return false;
        const href = $(el).attr('href');
        return matchesHrefPattern(href);
      }).map((_, el) => {
        const $el = $(el);
        return {
          id: $el.closest('[data-id]').attr('data-id') || $el.attr('id') || '',
          title: ($el.attr('data-title') || $el.text() || '').trim().slice(0, 160),
          href: $el.attr('href') || '',
        };
      }).get();
      const docCards = $('.document-list-view-documents').map((_, el) => {
        const $el = $(el);
        const anchor = $el.find('a[href*="/document/"]').first();
        return {
          id: $el.attr('data-id') || $el.attr('id') || '',
          title: ($el.attr('data-title') || anchor.text() || '').trim().slice(0, 160),
          href: anchor.attr('href') || '',
        };
      }).get();
      const dataDocs = $('[data-type]').filter((_, el) => {
        if (!this.resourceGuard.enabled) return false;
        const val = ($(el).attr('data-type') || '').toLowerCase();
        return this.resourceGuard.dataTypes.includes(val);
      }).map((_, el) => {
        const $el = $(el);
        const href = $el.find('a[href*="/document/"]').attr('href') || '';
        return {
          id: $el.attr('data-id') || $el.attr('id') || '',
          title: ($el.attr('data-title') || '').trim().slice(0, 160),
          href,
        };
      }).get();
      const mergedDocs = [...anchorDocs, ...dataDocs, ...docCards]
      ...
      // dedupe by href+title to avoid spam
      const seen = new Set();
      const docItems = [];
      ...
      const sampleLi = $('li').slice(0, 5).map((_, el) => $(el).text().trim().slice(0, 80)).get();
      // Fallback: parse the page's embedded initialDocumentList JSON if present
      const scriptDocsRaw = extractInitialDocumentList(html);
      const scriptDocs = scriptDocsRaw.map(d => ({
        id: d.Id || d.id || '',
        title: d.Title || d.title || '',
        href: d.ContentUrl || d.ContentURL || '',
      }));
      ...
      const docLinks = docItems.filter(d => d.href).length;
      this._debugLog('DOM list stats', {
        liCount,
        ulCount,
        maxUlSize,
        docLinks,
        docItems: docItems.length,
        scriptDocs: scriptDocs.length,
        anchorDocs: anchorDocs.length,
        sampleLi,
      });

      // Provide the agent an explicit, structured list of resources to avoid stopping early.
      if (this.resourceGuard.enabled && docItems.length) {
        const summary = [
          `Testronaut document list (${docItems.length} items):`,
          ...docItems.map(d => `- [${d.id || 'n/a'}] ${d.title || ''} ${d.href || ''}`.trim()),
        ].join('\n');
        $('body').prepend(`<pre data-testronaut-doc-list="true">${summary}</pre>`);
      }
    }
  ...
  async list_local_files({ dir = 'missions/files', extensions = [] } = {}) {
    const resolvedDir = path.resolve(process.cwd(), dir);
    if (!fs.existsSync(resolvedDir)) {
      throw new Error(`list_local_files: directory does not exist: ${resolvedDir}`);
    }
...
export const CHROME_TOOL_MAP = {
  ...
  resource_progress: (b, args, agentMemory) => {
    ...
    const remaining = prog.items.filter(i => !prog.downloaded.has(i.id));
    return JSON.stringify({
      enabled: true,
      total: prog.items.length || prog.lastScriptCount || 0,
      downloaded: prog.downloaded.size || 0,
      remaining: remaining.map(r => ({ id: r.id, title: r.title, href: r.href })).slice(0, 20),
